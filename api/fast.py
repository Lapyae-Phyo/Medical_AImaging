...
from api.grad_cam import plot_gradcam
from api.shap_image import generate_shap_image
...
async def predict_organ(file: UploadFile = File(...)):
    ...
    if img_array is not None:
        ...
        model_path = os.path.join(
            os.getcwd(), 'models', f'../models/{organ}_bin_model.h5')
        disease_model = load_model(model_path)

        disease_prediction = disease_model.predict(img_array)[0][0]
        shap_image = generate_shap_image(
            model=organ_detection_model, image=img_array)
        if disease_prediction >= 0.5:
            class_model_path = os.path.join(
                os.getcwd(), 'models', f'../models/{organ}_class_model.h5')
            ...
            if organ == 'knee':
                ...
                class_labels = ['cord_pathology', 'cystic_lesions',
                                'disc_pathology', 'osseous_abn']

            elif organ == 'lung':
                class_labels = ['airspace_opacity', 'bronchiectasis', 'nodule',
                                'parenchyma_destruction', 'interstitial_lung_disease']

            grad_image = plot_gradcam(
                class_model, img_array, layer_name='conv1_relu')
            app.state.grad_image = grad_image
            grad_image2 = plot_gradcam(
                class_model, img_array, layer_name='conv2_block1_1_conv')
